// Simple Node.js script to create PNG icons from our design
// This creates basic colored squares with brain emoji as fallback
...
<body>
    <h3>Right-click each canvas and "Save image as..." to save the icons</h3>
    
    <div>
        <p>icon16.png:</p>
        <canvas id="canvas16" width="16" height="16" style="border: 1px solid #ccc; image-rendering: pixelated;"></canvas>
    </div>
    ...
    <script>
        function drawIcon(canvas, size) {
            ...
            // For smaller sizes, use simpler design
            if (size <= 16) {
                ctx.fillStyle = '#1a73e8';
                ctx.fillRect(0, 0, size, size);
                ctx.fillStyle = '#ffffff';
                ...
                ctx.beginPath();
                ctx.ellipse(size/2, size/2, size/3, size/4, 0, 0, 2 * Math.PI);
                ctx.stroke();
...
fs.writeFileSync('./icons/generate-icons.html', iconHTML);
console.log('Created icons/generate-icons.html - open this file in your browser to download the icon files');